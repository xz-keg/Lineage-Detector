...
def get_union(variant_list):
    mut_in_union=[]
    
    mut_in_union=copy.deepcopy(variant_mutation_dic[variant_list[0]])
    for item in variant_list:
        this_mut=variant_mutation_dic[item]
        new_union=copy.deepcopy(mut_in_union)
        for mut in mut_in_union:
            if not(mut in this_mut):
                new_union.remove(mut)
        mut_in_union=new_union
    return mut_in_union
    
def get_difference(variant1,variant2):
    mut1=variant_mutation_dic[variant1]
    mut2=variant_mutation_dic[variant2]
    mut_dif=[]
    exist_pos=[]
    for item in mut2:
        if not(item in mut1):
            mut_dif.append(item)
            exist_pos.append(int(item[1:-1]))
            
    for item in mut1:
        if not(item in mut2):
            position=int(item[1:-1])
            if not(position in exist_pos):
                inverse_mut=item[-1]+item[1:-1]+item[0]
                mut_dif.append(inverse_mut)
            else:
                for i in range(len(mut_dif)):
                    if int(mut_dif[i][1:-1])==position:
                        mut_dif[i]=item[-1]+mut_dif[i][1:]
            
    mut_dif.sort(key=lambda x:int(x[1:-1]))
    return mut_dif
...
variant_mutation_dic['B.1.1.529']=get_union(['BA.1','BA.2'])
print(variant_mutation_dic['B.1.1.529'],len(variant_mutation_dic['B.1.1.529']))

difference=get_difference('B.1.1.529','BA.2.86')
print(difference,len(difference))

difference=get_difference('BA.2.86','NB.1.8.1')
print(difference,len(difference))

difference=get_difference('EG.5.1','JN.1')
print(difference,len(difference))
