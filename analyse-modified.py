		...
		if 'children' in node:
			for child in node['children']:
				...
				for ct in list(ret_info[4]):
					if ct in list(exmut_list):
						exmut_list[ct]+=ret_info[4][ct]
					else:
						exmut_list.update({ct:ret_info[4][ct]})
		...
		if is_terminal_lineage:
			...
			if (count>=important_threshold or (len(country_list)>=important_country and count>=2 and len(date_list)>=2) or (count>=2 and n_glycan)):
				...
				if len(imp_mut)>0:
					...
					w=highlight_browser(node)
	return [lineage,count,copy.deepcopy(country_list),copy.deepcopy(date_list),copy.deepcopy(exmut_list)]
